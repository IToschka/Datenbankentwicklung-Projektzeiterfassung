...
<html>
   ...
   <title>Bestehendes Projekt ändern</title>
    ...
    <body>
        ...
        <p>ProjektID:<input type="number" name="projectID" value= '<?php echo htmlentities($_SESSION['projectID']); ?>' required ></p>
       ...
       <form method="POST" action="projectsAndTasksChange.php">
        ...
        if(isset($_POST['button_change'])) {
            $task = $_POST['task'];
            $projectTaskID = getTaskID($conn, $_SESSION['projectID']);
            $projectTaskID++;
            $projectID = $_SESSION['projectID'];

            createTasks($conn, $projectTaskID, $projectID, $task);
        }
                ?>
